...
//Mendeklarasikan Variabel Variabel Yang Digunakan Selama Membuat Program
...
void buatakun(){//Menu Untuk Membuat Akun
	logo();
...
void jadwaldanpesan(){ //Menu Untuk Melihat Jadwal Pemesanan Dan Memesan Tiket
	logo();
...
void jam(int citi[],int c){ //Menu yang akan menampilkan jam penerbangan
...
void fakecity(int fcit[],int b){ //Menu Yang akan menampilkan kota yang telah di pilih/pesan
...
void maskapai(int pswt[],int d){ // Menu Yang akan menampilkan Maskapai Yang telah di pilih/pesan
...
int hargaterbang(int city[],int a){ // Mmenu Yang Akan Menetapkan Harga penerbangan di Setiap Maskapai yang dipilih
...
void rincian(){ //Menu Yang Akan Menampilkan Rincian Dari setiap pemesanan yang dipilih
	logo();
...
void bayar(){ //Menu yang akan mengeksekusi pembayaran
	logo();
...
void againmenu(){ //Menu Yang Akan mengembalikan program ke menu utama atau pun keluar dari program
    char pilih;
    scanf("%s", &pilih);
    if( pilih=='y' || pilih=='Y' )
    {
...
void reset(){ //Menu Yang Akan menghilangkan data data pemesanan
	logo();
    for (i=0;i<100;i++){
        price[i]=0;
        tanggal[i]=0;
...
void lihattiket(){ // Menu Yang Akan menampilkan tiket yang telah di pesan
	logo();
...
void refund(int h){ // Menu yang akan mengeksekusi bagian refund pemesanan
	logo();
...
void out() //Menu yang akan memberhentikan Program
{
    return;
}
void help(){ //Menu Yang akan membantu user
	logo();
...
void faq(){ //Menampilkan pertanyaan yang sering di tanyakan oleh para user ke perusahaan
	logo();
    printf("----FAQ----\n\n");
...
void info(){ //Akan memberikan Info terkait Tujuan penerbangan
	logo();
...
void back() //Mengembalikan ke menu sebelumnya di menu help
{
    int a=1;
    scanf("%d", &a);
    system("cls");
...
int prog(){ //Mendeskripsikan Product Kami
	logo();
